"""
This module provides functionality to ingest data sheet adm into knowledge graph
"""
...
async def handle_INGEST_DATA_SHEET_ADM_INTO_NEO4J(
    is_api_event, payload
):  # pylint: disable=invalid-name
    """
    Asynchronously handles the ingestion of data sheets into Neo4j.
    """
    logger.info("INIT: Ingest adm into neo4j")
    ...
    if step_status_graph_ql["success"]:
        await update_graph_ql_status(plant_id, DatabaseStatus.INGESTING_DATA_SHEET_ADM)

    data_sheet_file_path = adm_path
    bucket_name = os.getenv("BUCKET")
    logger.info("INIT: Reading adm from %s/%s", bucket_name, data_sheet_file_path)
    adm = await fetch_file_via_adapter(bucket_name, data_sheet_file_path)
    try:
        database_name = await Neo4jConnection.get_database_name(plant_id)
        await ingest_data_sheet(adm, database_name, plant_id)
        step_status_graph_ql = await get_graph_ql_status(plant_id)
        if step_status_graph_ql["success"]:
            await update_graph_ql_status(
                plant_id, DatabaseStatus.INGESTING_DATA_SHEET_ADM_SUCCESSFUL
            )
        step_status = {"success": True, "data": {"plant_id": plant_id}}
    except (KeyError, IndexError, TypeError, ValueError) as e:
        logger.error("ERROR: Ingesting adm Failed: %s", e)
        await update_graph_ql_status(plant_id, DatabaseStatus.INGESTING_DATA_SHEET_ADM_FAILED)
        step_status = {"success": False, "error": str(e)}

    logger.info("DONE: Ingest data sheet adm into neo4j")
