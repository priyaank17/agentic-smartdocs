"""
This module is responsible for handling the EXTRACT_PROCESS_NARRATIVE_DATA event."""
...
async def handle_TRIGGERED_PROCESS_NARRATIVE_RAG_JUNIOR_DATA_ACTION(
    params,
):  # pylint: disable=invalid-name
    """handle_TRIGGERED_PROCESS_NARRATIVE_RAG_JUNIOR_DATA_ACTION"""
    logger.info("INIT: handle_TRIGGERED_PROCESS_NARRATIVE_RAG_JUNIOR_DATA_ACTION")
    try:
        document_id = params["document_id"]

        await update_document_status(document_id, "IN_PROGRESS")

        params["action"] = "EXTRACT_PROCESS_NARRATIVE_RAG_JUNIOR_DATA"
        ...
        return {
            "statusCode": 400,
            "body": json.dumps(
                {"success": False, "msg": f"Missing key in parameters: {str(e)}"}
            ),
        }
        ...
        return {
            "statusCode": 500,
            "body": json.dumps(
                {"success": False, "msg": f"Boto3 client error: {str(e)}"}
            ),
        }
    except Exception as e:  # pylint: disable=broad-exception-caught
        logger.error(
            "Unexpected error in handle_TRIGGERED_PROCESS_NARRATIVE_RAG_JUNIOR_DATA_ACTION: %s",
            str(e),
        )
        await update_document_status(params.get("document_id", "unknown"), "ERROR")
        return {
            ...
            "body": json.dumps(
                {
                    "success": False,
                    "msg": f"Unexpected error in "
                    f"handle_TRIGGERED_PROCESS_NARRATIVE_RAG_JUNIOR_DATA_ACTION: {str(e)}",
                }
            ),
        }
