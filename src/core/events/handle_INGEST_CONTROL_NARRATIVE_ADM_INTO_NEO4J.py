...
from src.knowledge_graph_data_ingestion import (
    ingest_control_narrative_document,
)
...
async def handle_INGEST_CONTROL_NARRATIVE_ADM_INTO_NEO4J(
    is_api_event, payload
):  # pylint: disable=invalid-name
    """
    Asynchronously handles the ingestion of process narrative documents into Neo4j.
    """
    logger.info("INIT: Ingest Control Narrative ADM into neo4j")
    ...
    try:
        ...
        ingest_control_narrative_document(adm, database_name, plant_id)
        step_status_graph_ql = await get_graph_ql_status(plant_id)
        if step_status_graph_ql["success"]:
    ...
    logger.info("DONE: Ingest Control Narrative ADM into neo4j")
