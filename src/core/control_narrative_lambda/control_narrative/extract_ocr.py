...
async def _get_bounding_box_data(bucket, input_path):
    ...
    for table_data in data["tables_data"]:
        ...
        filtered_data = _extract_bounding_box(ocr_data, page_id, table_bounding_box)
...
def _extract_bounding_box(data, page_id, bounding_box):
...
async def create_narrative_csv(bucket, input_data_path):
    ...
    data = await load_json_from_storage(bucket, input_data_path)

    for table_data in data["tables_data"]:
        page_id = table_data.get("page_id")
        image_path = input_data_path.replace("input_data.json", f"{page_id}")
        ocr_output_json_path = input_data_path.replace("input_data.json", f"{page_id}.json")
    ...
    csv_path = input_data_path.replace("input_data.json", "control_narrative.csv")
