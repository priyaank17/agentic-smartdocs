"""
This module contains functions to rename attributes with best match
and filter source and destination name property.
"""
...
def filter_unverified_connections(updated_received_assets_connections):
    ...
    for connection in updated_received_assets_connections:
        flowrate_verification = connection.get("flow_rate", {}).get(
            "verification", False
        )
        temperature_verification = connection.get("temperature", {}).get(
            "verification", False
        )
        pressure_value_verification = connection.get("pressure", {}).get(
            "verification", False
        )
        flow_rate_value = connection.get("flow_rate", {}).get("value")
        logger.info(f"Flow rate value: {flow_rate_value}")
        temperature_value = connection.get("temperature", {}).get("value")
        logger.info(f"Temperature value: {temperature_value}")
        pressure_value = connection.get("pressure", {}).get("value")
        a = (
            not flowrate_verification
            and not temperature_verification
            and not pressure_value_verification
        )
        b = (
            not flowrate_verification
            or not temperature_verification
            or not pressure_value_verification
        ) and (
    ...
    logger.info("Unverified connections cleared")
    updated_received_assets_connections.extend(filtered_connections)
    logger.info("Unverified connections extended")
...
def remove_verification(json_data):
    ...
    if isinstance(data, dict):
        ...
        for _, value in json_data.items():
            logger.info(f"Removing verification from {value}")
